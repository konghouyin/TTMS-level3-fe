			...
			<div class="faimg">
				...
				<div class="nei">
					...
					<div class="action">
						...
						<div class="buy" @click="jumpbuy()"> 特惠购票</div>
					</div>
					<div class="ping">
					...
					<div v-if="prenum==0">
						<div style="height: 380px; width: 300px;margin-left: 600px;margin-top: -260px;">
							<a :href="showmoviesrc2" style="width: 140px; height:100px ;margin-right: 10px;position: relative;">
								<img :src="showmovieimg2" style="width: 140px; height:100px" />
								<em></em>
							</a>
						</div>
					</div>
		...
		<div style="width: 100%;display: flex; justify-content: center;">
			<div class="clear" style="min-width: 1200px;">
				...
				<div style="width: 730px; ">
					<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" id="menu" active-text-color="#EF4238"
					 style="height: 49.6px;">
					...
					<div style="width: 100%; height: 258px; display: inline-block; margin-bottom: 51px;">
						...
						<el-image :src="psrc1" style="width: 465px; height: 256px;"></el-image>
					</div>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			Axios.send('/reportType/getreportType', 'get', {}).then(res => {
				...
				//拿到用户想看状态
				Axios.send('/api/userwant/get', 'get', {
		...
		methods: {
			chosewish() {
				if (this.love) {
					...
					/* 增加想看 */
					Axios.send('/api/userwant/add', 'post', {
						playId: this.$router.history.current.query.id,
					}).then(res => {
						console.log(res)
						if (res.style == '1')
						...
						console.log('userwantaddError', error)
					}).catch(err => {
						throw err
					})
				}
